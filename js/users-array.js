 ...
 */
const condition = users.every((user) => /[^@]+@[^\.]+\..+/.test(user.email));
 ...
 */
const classANetworks = users.filter(
  (user) => user.ip_address.split('.')[0] < 128
);

console.log(classANetworks);
...
const findIndex = users.findIndex(({ ip_address }) => {
...
const deleted = users.fill('deleted');
