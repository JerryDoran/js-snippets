...
console.log(accounts[0].tags.flat(2)); // go two levels deep into the parent array
 ...
 */
console.log(accounts[0].tags.flat().includes('consectetur'));
...
console.log(
  accounts
    .filter(
      ({ balance, age }) =>
        age <= 30 && parseInt(balance.replace(/\D/g, '')) > 1500
    )
    .map(({ friends }) => friends)
    .flat()
);
